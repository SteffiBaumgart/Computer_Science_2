...
//shared variables
			...
			pauseB.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (pauseB.getText().equals("Pause ")) {
						...
						//PersonMover.done = false; //stop people moving
						//counterD.done=false; //stop counter thread
						//w.repaint();
						//w.done=false; //stop animator thread
						//done = false;
						paused = false;
					}
	...
	public static void createThreads() {
        ...
        for (int i=0;i<noPeople;i++) {
      	...
      	Thread t = new Thread(w); //w is a roompanel (type Runnable)
      	t.start();
      	//Start counter thread - for updating counters
      	Thread s = new Thread(counterD); //counterD is a CounterDisplay (type Runnable)
      	s.start();
	}
